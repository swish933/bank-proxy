...
async function renderPDF(html) {
	...
	try {
		...
		try {
			...
			logger.warn({ closeErr }, "Failed to close page cleanly");
		}
	}
}
...
function buildPdfHtml(formState) {
	...
	const totalDocumentPages = 5 + (includesJoint ? 1 : 0) + 11;
	...
	p2Sections += compileBlock("Purpose & Referral Source", [
	...
	// ── Optional Page 3: Joint Account Holder (Synchronized Layout) ───────────
	let page3 = "";
	...
	if (includesJoint && formState.joint) {
		...
		let pJointSections = compileBlock("Joint Account Holder Profile", [
		...
		pJointSections += compileBlock("Identification (Joint Account Holder)", [
		...
		pJointSections += compileBlock(
			...
			[
				...
				{ label: "Occupation / Employment Segment", value: j.emp_segment },
				{ label: "Job Title", value: j.job_title },
				{
					label: "No. of years in present employment",
					value: j.years_employed,
				},
				...
				{ label: "WhatsApp No", value: j.whatsapp_number },
		...
		page3 = createPageHTML(
			...
			"To be completed by the second signatory",
			pJointSections,
			3,
			totalDocumentPages,
		);
	...
	let page4 = createPageHTML(
		...
		"Account names must match CSCS details",
		p3Sections,
		bankPageNum,
		totalDocumentPages,
	);
	...
	if (formState.rel_political === "Yes") {
		if (formState.rel_political_1?.name) {
			politicalArray.push(
				{ label: "I) Name", value: formState.rel_political_1.name },
				{
					label: "Relationship",
					value: formState.rel_political_1.relationship,
				},
				{ label: "Position Held", value: formState.rel_political_1.position },
				{
					label: "Date: From — To",
		...
		if (formState.rel_political_2?.name) {
			politicalArray.push(
				{ label: "II) Name", value: formState.rel_political_2.name },
				{
					label: "Relationship",
					value: formState.rel_political_2.relationship,
				},
				{ label: "Position Held", value: formState.rel_political_2.position },
				{
					label: "Date: From — To",
    ...
    <div class="nsl-acct-block">
      <div class="nsl-acct-row"><span class="nsl-acct-label">Account Name</span><span class="nsl-acct-value">NIGERIAN STOCKBROKERS LIMITED-CLIENTS ACCOUNT</span></div>
      <div class="nsl-acct-row"><span class="nsl-acct-label">Account Number</span><span class="nsl-acct-value">1000012745</span></div>
      <div class="nsl-acct-row"><span class="nsl-acct-label">Bank</span><span class="nsl-acct-value">GLOBUS BANK</span></div>
	  <div class="nsl-acct-row"><span class="nsl-acct-label">Initial Amount Deposited (&#x20A6;)</span><span class="nsl-acct-value">${formState.initial_deposit ? Number(formState.initial_deposit).toLocaleString("en-US") : "N/A"}</span></div>
    </div>
  </div>`;

	let page5 = createPageHTML(
		"Questionnaire",
		"Minor Details, Political Exposure & Portfolio Mandate",
		p4Sections,
		compliancePageNum,
		totalDocumentPages,
	);
	...
	let p5Sections = compileBlock("Attestation", [
		...
		{
			label: "I (We) attest all information provided is accurate",
	...
	const assetMap = [
		...
		{ data: formState.joint?.passport_photo, tag: "Joint Holder — Passport" },
		{ data: formState.joint?.signature, tag: "Signature of Joint Holder" },
		{ data: formState.joint?.id_doc, tag: "Joint Holder — Valid ID" },
		{
		...
		},
		{
			...
			tag: "NOK 1 — Passport Photograph",
		},
		{ data: formState.nok1?.id_doc, tag: "NOK 1 — Valid Means of ID" },
		{
			data: formState.nok2?.passport_photo,
			tag: "NOK 2 — Passport Photograph",
		},
		{ data: formState.nok2?.id_doc, tag: "NOK 2 — Valid Means of ID" },
		{
	...
	for (const asset of assetMap) {
		...
		if (asset.data === "[PDF_DOCUMENT]") {
			galleryHTML += `<div class="pdf-img-box"><span>${asset.tag}</span><div class="pdf-doc-placeholder">📄 PDF Document</div></div>`;
		} else {
			galleryHTML += `<div class="pdf-img-box"><span>${asset.tag}</span><img src="${asset.data}" alt="${asset.tag}" /></div>`;
		}
		assetCount++;
	}
	...
	if (assetCount > 0) {
      ...
      <div class="pdf-section-title">Uploaded Documents Gallery</div>
	...
	// ── Risk Disclosure Statement Pages ──────────────────────────────────────────

	// ── RDS Page 1: Introduction + Sections A (Investment & Market Risk) ─────────
	const rdsHTML1 = `
  <div class="static-section">
    <div class="static-section-title">Risk Disclosure Statement</div>
    <p class="static-para">Nigerian Stockbrokers Limited is registered with the Securities and Exchange Commission and licensed by the Nigerian Exchange Limited (NGX) to carry on securities trading business.</p>
    <p class="static-para">Generally, securities investments are suitable for clients who fully understand and appreciate the risks involved. It is highly speculative, involves a high degree of risk and may result in the loss of the entire investment. Therefore, before deciding to trade equities, debt securities, Exchange Traded Funds, Derivatives or engage in securities borrowing or lending and/or other related ancillary or capital market operations, Clients should carefully consider their objectives and risk appetite.</p>
    <p class="static-para">The Client should exercise particular care in evaluating the risks involved and must decide whether, in the light of the assessed risks, investment is appropriate. Clients should be aware that there are always significant risks involved when investing in securities in the capital market. Many unforeseen events such as new and existing government policies, domestic and international political and economic events, and other events of force majeure (acts of God, war, riot, urban unrest, terrorism, epidemic, pandemic, natural disasters etc.) may cause sharp market and price fluctuations. Clients are encouraged to consider all risks (material and non-material) as well as other qualitative and quantitative risk factors associated with investing in securities which differ across board.</p>
	<p class="static-para">Additionally, Clients should fulfil specific requirements laid out by the Trading License Holder, which are subject to guidelines, rules and regulations prescribed by the Securities and Exchange Commission, NGX and international best practices. Clients should note that any information contained in the Risk Disclosure Statement may also become outdated relatively quickly.</p>
    <p class="static-para">The Client should acknowledge and accept each of the risks listed and accept that all these risks could lead to the loss of some or the entire value of the Client's investments.</p>
  </div>

  <div class="static-section">
    <div class="static-section-title">A. Risks Associated with Securities Trading</div>
    <p class="static-para"><strong>1. Investment Risk</strong></p>
    <p class="static-para">All securities investments risk the loss of capital. Many unforeseeable events, including actions by various government agencies and domestic and international political and economic events and other events of force majeure, such as acts of God, and certain other events beyond reasonable control (e.g. war, riot, urban unrest, terrorism, epidemic, pandemic, natural disasters etc.), may cause sharp market and price fluctuations.</p>
    <p class="static-para"><strong>2. Market Liquidity</strong></p>
    <p class="static-para">Some listed securities in which the Client may invest may be traded in very low volumes; as such, there is the risk that investments may not be easily liquidated. The prices and values of securities can also be influenced by outside factors such as interest rate changes, price or level of any underlying asset, level of interest rates, foreign exchange rates, volatility and liquidity.</p>
    <p class="static-para"><strong>3. Currency Risk</strong></p>
    <p class="static-para">The potential for profit or loss from transactions in foreign currency-denominated securities (traded locally or in other jurisdictions) will be affected by fluctuations in foreign exchange rates. Clients or companies that have assets or business operations across national borders are exposed to currency risk that may create unpredictable profits and losses.</p>
    <p class="static-para"><strong>4. Execution Risk</strong></p>
    <p class="static-para">We, Nigerian Stockbrokers Limited acting as a broker for the purchase, sale and/or investment in the Securities, may, on certain occasions, not be able to execute the purchase or sale or to undertake any action relating to the securities in accordance with the Clients' instruction. These occasions include, amongst other things, a situation where the market conditions are not favorable, the order cannot be matched, the laws and regulations of the Exchange or other competent authorities impose limitations, etc.</p>
    <p class="static-para"><strong>5. Commission, Fees, Interest and Other Charges</strong></p>
    <p class="static-para">All regulatory fees, commissions, charges are as approved by the regulatory authorities. The Client should obtain a clear explanation of all commissions, fees, interest and charges, and understand that these charges may affect the Client's net profit (if any) or increase the Client's loss. The Client agrees that he/she will be liable for these charges (as may be amended from time to time).</p>
    <p class="static-para"><strong>6. Settlement Risk</strong></p>
    <p class="static-para">Settlement risk may arise as a result of technological issues or glitches within the infrastructure, resulting in a delay in executing clients' mandates.</p>
  </div>`;

	const pageRDS1 = createStaticPageHTML(
		"Risk Disclosure Statement (1 of 4)",
		"Introduction &amp; Securities Trading Risks",
		rdsHTML1,
		staticBase + 4,
		totalDocumentPages,
	);

	// ── RDS Page 2: Sections B (Debt Securities) + C (ETFs) ─────────────────────
	const rdsHTML2 = `
  <div class="static-section">
    <div class="static-section-title">B. Risks Associated with Debt Securities</div>
    <p class="static-para">Debt Securities and Debt-linked investments offer fixed returns over a defined period and may be intended to be held to maturity. These instruments carry a significant amount of risk such as credit, currency and liquidity risks.</p>
    <p class="static-para">Credit risk in debt securities arise from default events that may result in the inability of the issuer to pay interest or principal.</p>
	<p class="static-para"> Default risk in debt securities is high when credit rating is non-investment grade or non-rated. In a default situation, the Client may lose both interest and principal.</p>
  </div>

  <div class="static-section">
    <div class="static-section-title">C. Risks Associated with Exchange Traded Funds (ETFs)</div>
    <p class="static-para"><strong>1. Market Risk</strong></p>
    <p class="static-para">Exchange Traded Funds (ETFs) are exposed to the economic, political, currency, legal, tax and other risks of a specific factor or market related to the ETFs or the index and the market that it is tracking.</p>
    <p class="static-para"><strong>2. Currency Risk</strong></p>
    <p class="static-para">Clients will encounter currency risk where the exchange traded investments' underlying holdings are in a currency which is different to the denominated currency.</p>
    <p class="static-para"><strong>3. Liquidity Risk</strong></p>
    <p class="static-para">ETFs usually have market makers to provide liquidity, however, there is no guarantee that active trading will be maintained frequently. Therefore, where market makers are unable to fulfil their obligations, Clients may be unable to buy or sell the ETF and increase the risk of loss.</p>
    <p class="static-para"><strong>4. Counterparty Risk</strong></p>
    <p class="static-para">This is the risk that the value of the collateral falls below the Net Asset Value (NAV) of the fund when the fund counterparty is in default.</p>
    <p class="static-para"><strong>5. Tracking Error Risk</strong></p>
    <p class="static-para">A disparity between the performance of the ETF measured by its Net Asset Value (NAV) and the performance of the underlying index may occur due to several factors such as the failure of the ETF's tracking strategy, fees and expenses, foreign exchange differences between the base currency or trading currency of the ETF and the currencies of the underlying investments, or corporate actions of the ETF etc. The performance of the securities underlying the ETF as measured by its NAV may outperform or underperform the underlying index.</p>
    <p class="static-para"><strong>6. Index Risk</strong></p>
    <p class="static-para">ETFs are not actively managed, therefore, a security will not be sold where the security's issuer is in financial trouble unless the security is removed from the index. The best approach for an investor to deal with an index risk is to understand the index and the rules governing the index.</p>
    <p class="static-para"><strong>7. Risks of Trading at a Discount or Premium</strong></p>
    <p class="static-para">The trading price of an ETF is determined by the supply and demand factors which emerge during periods of market volatility and uncertainty. Clients that buy at a premium may suffer losses even if the NAV is higher when they sell, and they may not fully recover their investment in the event of termination of the ETF.</p>
  </div>`;

	const pageRDS2 = createStaticPageHTML(
		"Risk Disclosure Statement (2 of 4)",
		"Debt Securities &amp; Exchange Traded Fund Risks",
		rdsHTML2,
		staticBase + 5,
		totalDocumentPages,
	);

	// ── RDS Page 3: Sections D (Derivatives) + E (Margin) + F (Securities Lending) ──
	const rdsHTML3 = `
  <div class="static-section">
    <div class="static-section-title">D. Risks Associated with Exchange Traded Derivatives Products</div>
    <p class="static-para"><strong>1. Extraordinary Price Movements</strong></p>
    <p class="static-para">The price of an exchange traded derivatives product may not match its theoretical price due to market supply and demand factors. As a result, actual traded prices can be higher or lower than the theoretical price.</p>
    <p class="static-para"><strong>2. Foreign Exchange Risk</strong></p>
    <p class="static-para">Clients trading exchange traded derivatives products with underlying assets in other currencies are exposed to exchange rate risk. Currency rate fluctuations can adversely affect the underlying asset value, also affecting the exchange traded product price.</p>
    <p class="static-para"><strong>3. Expiry Considerations</strong></p>
    <p class="static-para">Most of the exchange traded derivatives products have an expiry date after which the issue may become worthless. Clients should be aware of the expiry period and choose a product with an appropriate lifespan for their trading strategy.</p>
    <p class="static-para"><strong>4. Gearing Risk</strong></p>
    <p class="static-para">Exchange traded derivatives products are leveraged and can change in value rapidly according to the gearing ratio relative to the underlying assets. Clients should be aware that the value of an exchange traded product may fall to zero resulting in a total loss of the initial investment.</p>
    <p class="static-para"><strong>5. Issuer Default Risk</strong></p>
    <p class="static-para">Where an exchange traded derivatives product issuer becomes insolvent and defaults on their listed securities, Clients will be considered as unsecured creditors and will have no preferential claims to any assets held by the issuer. Clients should thoroughly review the financial strength and credit worthiness of exchange traded derivatives product issuers.</p>
    <p class="static-para"><strong>6. Uncollateralized Product Risk</strong></p>
    <p class="static-para">Uncollateralized exchange traded derivatives products are not asset backed and where there is issuer bankruptcy, Clients can lose their entire investment. Clients should read the listing documents to determine if a product is uncollateralized.</p>
  </div>

  <div class="static-section">
    <div class="static-section-title">E. Risk of Margin Trading (e.g. Share Margin Financing)</div>
    <p class="static-para">The risk of loss in financing a transaction by deposit of collateral may be significant. The Client may sustain losses in excess of his/her cash and any other assets deposited as collateral. The Client may be called upon at short notice to make additional margin deposits or interest payments. If required margin deposit or interest payment is not made within the prescribed time, the Client's collateral or positions may be liquidated at a loss without prior notification to the Client. The Client should therefore carefully consider whether such a financing arrangement with the firm is suitable in light of its own financial position and investment objectives.</p>
  </div>

  <div class="static-section">
    <div class="static-section-title">F. Securities Borrowing and Lending</div>
    <p class="static-para">When the Client borrows securities, he/she will be required to deposit a required level of collateral. The Client may be called upon on short notice to place additional deposits if the level of collateral is inadequate in relation to the market value of borrowed securities. If the required deposit is not made within the prescribed time, the firm may buy-back the borrowed securities without prior notification to the Client.</p>
    <p class="static-para">When the Client lends securities to the firm, the Client temporarily loses legal ownership rights to the securities but has a right to claim equivalent securities and dividends.</p>
  </div>`;

	const pageRDS3 = createStaticPageHTML(
		"Risk Disclosure Statement (3 of 4)",
		"Derivatives, Margin Trading &amp; Securities Lending Risks",
		rdsHTML3,
		staticBase + 6,
		totalDocumentPages,
	);

	// ── RDS Page 4: Sections G–J + Acknowledgement signature block ───────────────
	const rdsHTML4 = `
  <div class="static-section">
    <div class="static-section-title">G. Risk Relating to the Investor's Own Investment</div>
    <p class="static-para">Each decision by a Client to invest in the securities is his/her own independent decision. The firm is not acting as an advisor. Before making a decision to invest, Clients should have confirmed that they have carefully studied and considered all information relating to the status, business, financial condition and operation of the company/entity issuing the securities, including the underlying assets of or other information relating to the securities in which the Clients will invest, and other relevant factors relating to the investment such as political, economic, legal and regulatory conditions, as well as market conditions, demand, supply and price of the underlying goods of the securities. As a result, the Clients bear all risks from the investment.</p>
  </div>

  <div class="static-section">
    <div class="static-section-title">H. Regulatory Risk</div>
    <p class="static-para">Nigerian Stockbrokers Limited is subject to the various laws and/or regulations of the competent relevant regulatory authorities. Any legislative or regulatory changes may impact the functions of the broker/dealer. All transactions performed by ("name of the Trading License Holder") will be executed in accordance with the relevant rules and regulations and any that fall contrary to the existing regulatory framework may be nullified by the regulatory authorities.</p>
  </div>

  <div class="static-section">
    <div class="static-section-title">I. Non-Advisory Nature of Relationship</div>
    <p class="static-para">Unless the Client has a specific agreement with (“name of the Trading License Holder) for the provision of advisory services or fund management services, the Client should note and accept that the relationship with Nigerian Stockbrokers Limited in relation to the Client's securities and securities-related transactions is purely execution only.</p>
    <p class="static-para">Whilst the firm will exercise all due care and skill at all times in the course of carrying out capital market services on behalf of the client, all final investment decisions are taken by the Client and not the firm. The Trading License Holder merely acts in a non-advisory capacity and executes orders on behalf of the Client unless otherwise specifically stated.</p>
  </div>

  <div class="static-section">
    <div class="static-section-title">J. Risk Relating to Digital Trading</div>
    <p class="static-para">A Client can decide to trade electronically through the platform of his/her broker on his/her own account thereby taking responsibility for his / her action. Trading via digital channels is subject to certain risks including, but not limited to, technological glitches, time-lag between order entry and real execution due to delay transmission, password compromise, viruses and other possible harmful elements that could corrupt, compromise, harm or otherwise impede the operation or performance of a computer system or network. Clients should ensure that they go through the terms of service applicable to any online service they wish to subscribe to.</p>
    <p class="static-para">Clients should also ensure that they maintain appropriate security measures in the operation of their online accounts including using strong passwords, keeping their passwords and log in details safe, avoiding log in from public computers and keeping their computers protected from viruses, etc.</p>
  </div>`;

	const pageRDS4 = createStaticPageHTML(
		"Risk Disclosure Statement (4 of 4)",
		"Investor Responsibility, Regulatory &amp; Digital Risks",
		rdsHTML4,
		staticBase + 7,
		totalDocumentPages,
	);
    ...
    <div class="static-section" style="margin-top:6px;">
      <div class="static-section-title">Visitation/Verification Report</div>
	...
	const officialHTML = `
    <div class="static-section">
      <div class="static-section-title">For Official Use Only (Document Checklist)</div>
    ...
    <div class="static-section" style="margin-top:6px;">
      <div class="static-section-title">For Official Use Only (Account Opening Type)</div>
...
<head>
  ...
  <title>NSL Application Portfolio</title>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; text-rendering: optimizeLegibility }
    body { font-family: sans-serif; font-synthesis: none; color: #000000; background: #ffffff; -webkit-print-color-adjust: exact; print-color-adjust: exact; font-weight: 400; font-smooth: always; }
    .pdf-page { width: 210mm; min-height: 297mm; padding: 14mm 15mm 12mm 15mm; page-break-after: always; background: #ffffff; position: relative; display: flex; flex-direction: column; }
    .pdf-page:last-of-type { page-break-after: avoid; }
    .pdf-header { display: flex; justify-content: space-between; align-items: flex-end; border-bottom: 2px solid #8b1a1a; padding-bottom: 8px; margin-bottom: 15px; flex-shrink: 0; }
    .pdf-header-left h1 { color: #8b1a1a; margin: 0 0 4px 0; font-size: 20px; font-family: serif; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; }
    ...
    .pdf-section { margin-bottom: 6px; border: 1px solid #e2ddd6; border-radius: 6px; background: #ffffff; overflow: hidden; break-inside: avoid; }
    .pdf-section-title { font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: #ffffff; background: #8b1a1a; padding: 6px 12px; font-weight: 700; }
    .pdf-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px 15px; padding: 12px; background: #faf9f7; }
    .pdf-field { font-size: 11px; line-height: 1.4; }
    .pdf-field strong { color: #7a7068; display: block; font-size: 9px; text-transform: uppercase; margin-bottom: 2px; letter-spacing: 0.3px; }
    .pdf-field span { font-size: 12px; color: #000000; font-weight: 600; word-break: break-word; }
    .pdf-full-row { grid-column: span 2; }
    .pdf-gallery { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; padding: 12px; background: #faf9f7; }
    .pdf-img-box { border: 1px solid #e2ddd6; padding: 8px; border-radius: 4px; text-align: center; background: #ffffff; break-inside: avoid; }
    .pdf-img-box img { max-width: 100%; height: 100px; object-fit: contain; display: block; margin: 6px auto 0; border-radius: 2px; }
    .pdf-img-box span { font-size: 9px; font-weight: 700; color: #4a4238; text-transform: uppercase; display: block; border-bottom: 1px solid #f0ede9; padding-bottom: 4px; }
    .pdf-doc-placeholder { font-size: 10px; padding: 35px 5px; color: #7a7068; font-weight: bold; background: #f0ede9; margin-top: 6px; border-radius: 2px; text-transform: uppercase; }
    .pdf-footer { margin-top: auto; padding-top: 8px; display: flex; justify-content: space-between; border-top: 1px solid #e2ddd6; font-size: 9px; color: #7a7068; flex-shrink: 0; }
    @page { size: A4; margin: 0; }
    .nsl-acct-block { padding: 8px 12px 10px; background: #faf9f7; display: flex; flex-direction: column; gap: 5px; }
    .nsl-acct-row { display: flex; align-items: baseline; gap: 10px; font-size: 11px; }
    .nsl-acct-label { font-size: 9px; text-transform: uppercase; color: #7a7068; font-weight: 700; letter-spacing: 0.3px; min-width: 110px; flex-shrink: 0; }
    .nsl-acct-value { font-size: 12px; color: #1c1814; font-weight: 700; }
    .pdf-static-body { flex: 1; overflow: hidden; }
    .static-section { margin-bottom: 6px; border: 1px solid #e2ddd6; border-radius: 6px; overflow: hidden; break-inside: avoid; }
    .static-section-title { font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: #ffffff; background: #8b1a1a; padding: 5px 12px; font-weight: 700; }
    .static-para { font-size: 10.5px; line-height: 1.5; color: #1c1814; padding: 0 12px; margin-top: 5px; }
    .static-para:last-of-type { padding-bottom: 8px; }
    .static-list { font-size: 10.5px; line-height: 1.5; color: #1c1814; padding: 4px 12px 8px 30px; }
    .static-list li { margin-bottom: 3px; }
    .static-list.alpha { list-style-type: lower-alpha; }
    .sign-row { display: flex; gap: 20px; padding: 10px 12px 14px; background: #faf9f7; }
    .sign-block { flex: 1; }
    .sign-block-wide { flex: 2; }
    .sign-block-narrow { flex: 1; }
    .sign-label { font-size: 9px; text-transform: uppercase; color: #7a7068; font-weight: 700; letter-spacing: 0.3px; margin-bottom: 20px; display: block; }
    .sign-line { border-bottom: 1px solid #1c1814; height: 22px; }
    .memo-row { padding: 6px 12px; background: #faf9f7; }
    .memo-field { margin-bottom: 14px; }
    .aml-options { padding: 6px 12px 0; background: #faf9f7; }
    .aml-opt-row { display: flex; align-items: flex-start; gap: 8px; margin-bottom: 6px; font-size: 10.5px; }
    .aml-checkbox { display: inline-block; width: 12px; height: 12px; border: 1.5px solid #8b1a1a; border-radius: 2px; flex-shrink: 0; margin-top: 1px; }
    .aml-opt-label { line-height: 1.45; color: #1c1814; }
    .aml-classification { padding: 0 12px 10px; background: #faf9f7; font-size: 10.5px; display: flex; align-items: center; gap: 18px; }
    .aml-class-item { display: inline-flex; align-items: center; gap: 5px; }
    ...
    .visit-cells { display: flex; flex-wrap: wrap; gap: 2px; margin-top: 5px; }
    .vcell { width: 18px; height: 18px; border: 1px solid #c8b8a8; border-radius: 2px; background: #fff; }
    .official-table { width: 100%; border-collapse: collapse; font-size: 10.5px; background: #faf9f7; margin: 0; }
    .official-table th { background: #f0ede9; color: #4a4238; font-size: 9px; text-transform: uppercase; letter-spacing: 0.3px; padding: 5px 10px; border-bottom: 1px solid #e2ddd6; text-align: left; }
    .official-table td { padding: 5px 10px; border-bottom: 1px solid #f0ede9; color: #1c1814; }
    .off-checkbox { display: inline-block; width: 13px; height: 13px; border: 1.5px solid #8b1a1a; border-radius: 2px; vertical-align: middle; }
    .official-sign-grid { padding: 10px 12px 14px; background: #faf9f7; display: flex; flex-direction: column; gap: 10px; }
    .official-sign-row { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
      /* Opening type */
    .opening-type-grid {
      display: grid;
